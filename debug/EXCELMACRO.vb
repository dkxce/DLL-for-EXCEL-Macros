...
' Либо из двух столбцов, во второй столбец будет внесен результат из двух левых ячеек (текущая - предыдущая)
    ...
    dlgRes = MsgBox("Обработано точек скриптом: " & res & vbNewLine & "Изменено ячеек скриптом: " & changed & vbNewLine & "Результат это расстояние в метрах." & vbNewLine & "Заполнить правый столбец выбоки полученным результатом?", vbYesNo + vbQuestion, "GetLengthBetween2Points")
    If dlgRes = vbYes Then
        ' Заполнениее выделенных ячеек в таблице
        For Each cel In selectedRange.Cells
...
' Вызов скрипта SearchAddressOSM из библиотеки
...
' Сохранение результата 55.555555,33.333333 в исходные ячейки
    ...
    dlgRes = MsgBox("Обработано ячеек скриптом: " & res & vbNewLine & "Изменено ячеек скриптом: " & changed & vbNewLine & "Заполнить выбранные ячейки полученным результатом?", vbYesNo + vbQuestion, "SearchAddressOSM")
    If dlgRes = vbYes Then
        ' Заполнениее выделенных ячеек в таблице
        For Each cel In selectedRange.Cells
            Cells(cel.Row, cel.column).FormulaR1C1 = GetCellFormula(cel.Row, cel.column)
